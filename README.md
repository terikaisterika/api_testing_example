...
<h3>REST API</h3>
<h4>Видео: https://drive.google.com/file/d/1aKflXDXPe5TGLwdmDubWL3-6bFu3Cb25/view?usp=sharing</h4>
...
<h4>Коллекции rest запросов для Postman будут в проекте.</h4>
<p>На видео в коллекциях ws есть наименования отправитель и получатель, либо новенький/остальная команда. Сделано для того, чтобы проще было усвоить, как это работает. Но в принципе любой запрос может выполнять любую роль.</p>
...
<h4>Видео: https://drive.google.com/file/d/1wl2WD1lrmRIGl6zZ0isZD8UTjsY7sCVA/view?usp=sharing</h4>
...
<h4>Видео: https://drive.google.com/file/d/1qDNI2ZDVfcQYpXVV5dX5QwzIMi5hwI2i/view?usp=sharing</h4>
...
<h4>Видео: https://drive.google.com/file/d/1NmbhbuUT98JjMiEY_pnhsCLH55ty29ct/view?usp=sharing</h4>
<p>Проект можно запустить перейдя в папку cd .\get-information\ =>npm run start_ws. Подключение будет работать только если есть параметр <strong>token=123</strong>. Понятное дело, что это не авторизация. Просто добавлено, чтобы не было прям совсем скучно.</p>
<img src="https://github.com/terikaisterika/api_testing_example/assets/48588741/cad1627e-c893-47fc-a3ef-0ae95e36d6c3"/>
...
<h3>К сожалению, приложить коллекцию запросов к проектам socket.io и websocket не получится. Postman не дает ее экспортировать. В папке collections есть коллекция REST. Но я постараюсь сделать инструкцию не так размазанно, как до этого (просто там было много пояснений).</h3>
<h3>Для тех, кто сначала делает, а потом смотрит, почему не получилось.</h3>
<p>1.Установка проекта через git clone.</p>
<p>2.В проекте task-api и get-information сделать npm install отдельно.</p>
<h4>***REST проект***</h4>
<p>3.Из папки collections взять файл и импортировать в Postman (там же уже стоит базовая авторизация). Коллекция поделена на 2 роли. admin и manager по базовой авторизации (у менеджера нет прав на редактирование и удаление юзеров.</p>
<p>4.Команда запуска проекта из папки task-api npm start.</p>
<p>5.В access.log есть какие-никакие логи (просто для общего ознакомления).</p>
<h4>***проект c socket.io***</h4>
<p>6.Запуск проекта из папки get-information c помощью команды npm start.</p>
<p>7.Адрес ws://127.0.0.1:7000 для взаимодействия: с сервером через готовые команды, с rest api через готовые команды (запросить количество задач и юзеров) (описание команд выше).</p>
<p>8.Адрес ws://127.0.0.1:7000/work для взаимодействия по комнатам и пространствам.</p>
<h4>***проект c websocket***</h4>
<p>9.Запуск проекта из папки get-information c помощью команды npm run start_ws</p>
<p>10.Подключение по ws://127.0.0.1:7001 с обязательным параметром token=123. Можно запустить второе подключение через ws://127.0.0.1:7001 и отправлять сообщения, чтобы посмотреть, как это работает.</p>
<p>11.Можно воспользоваться командами взаимодействия с сервером. (описаны выше)</p>
