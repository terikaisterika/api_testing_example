...
1.Установка проекта через git clone.
2.В проекте task-api и get-information сделать npm install отдельно
<h4>***REST проект***</h4>
3.Из папки collections взять файл и импортировать в Postman (там же уже стоит базовая авторизация). Коллекция поделена на 2 роли. admin и manager по базовой авторизации (у менеджера нет прав на редактирование и удаление юзеров.
4.Команда запуска проекта из папки task-api npm start
5.В access.log есть какие-никакие логи (просто для общего ознакомления)
<h4>***проект c socket.io***</h4>
6.Запуск проекта из папки get-information c помощью команды npm start
7.Адрес ws://127.0.0.1:7000 для взаимодействия: с сервером через готовые команды, с rest api через готовые команды (запросить количество задач и юзеров) (описание команд выше);
8.Адрес ws://127.0.0.1:7000/work для взаимодействия по комнатам и пространствам.
<h4>***проект c websocket***</h4>
9.Запуск проекта из папки get-information c помощью команды npm run start_ws
10.Подключение по ws://127.0.0.1:7001 Можно запустить второе подключение через ws://127.0.0.1:7001 и отправлять сообщения, чтобы посмотреть, как это работает.
11.Можно воспользоваться командами взаимодействия с сервером. (описаны выше)
